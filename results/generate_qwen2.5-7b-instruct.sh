...
do 
    echo "正在评估 $dataset ..."
    mkdir -p "/root/prm/results/qwen2.5-7b-instruct"
    python results/generate.py \
        --dataset "$dataset" \
        --model "qwen2.5-7b-instruct" \
        --batch_size 25 \
        --n_responses 4 \
        --output "/root/prm/results/qwen2.5-7b-instruct/$(basename $dataset).json" \
        --interval 10 \
    > "/root/prm/results/qwen2.5-7b-instruct/generate_$(basename $dataset).log"
done
